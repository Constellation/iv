...
TEST(unique_ptrCase, unique_ptrTest) {
  bool target = false;
  {
    iv::core::unique_ptr<Checker> ptr(new Checker(&target, 0));
  }
  EXPECT_TRUE(target);
}
...
TEST(unique_ptrCase, ScopedArrayTest) {
  std::array<bool, 10> results;
  {
    iv::core::unique_ptr<Checker[]> ary(new Checker[10]);
