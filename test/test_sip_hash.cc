...
#include <cstdio>
#include <cstring>
#include <iv/sip_hash.h>

using namespace iv::core;  // NOLINT
...
*/
static const uint8_t SPEC_VECTORS[64][8] =
{
...
TEST(SipHashCase, test_24_spec) {
  uint64_t digest64;

  digest64 = SipHash::Hash24(SPEC_KEY, SPEC_MSG, 15);
  EXPECT_EQ(digest64, 0xa129ca6149be45e5ULL);
}

TEST(SipHashCase, test_24_empty_string) {
  uint64_t digest64;

  digest64 = SipHash::Hash24(SPEC_KEY, (uint8_t *) "", 0);
  EXPECT_EQ(digest64, 0x726fdb47dd0e0e31ULL);
}

TEST(SipHashCase, test_24_one_byte) {
  uint64_t digest64;

  digest64 = SipHash::Hash24(SPEC_KEY, (uint8_t *) "a", 1);
  EXPECT_EQ(digest64, 0x2ba3e8e9a71148caULL);
}

TEST(SipHashCase, test_24_six_bytes) {
  uint64_t digest64;

  digest64 = SipHash::Hash24(SPEC_KEY, (uint8_t *) "abcdef", 6);
  EXPECT_EQ(digest64, 0x2a6e77e733c7c05dULL);
}

TEST(SipHashCase, test_24_seven_bytes) {
  uint64_t digest64;

  digest64 = SipHash::Hash24(SPEC_KEY, (uint8_t *) "SipHash", 7);
  EXPECT_EQ(digest64, 0x8325093242a96f60ULL);
}

TEST(ShipHashCase, test_24_eight_bytes) {
  const uint64_t digest64 = SipHash::Hash24(SPEC_KEY, (uint8_t *) "12345678", 8);
  EXPECT_EQ(digest64, 0x2130609caea37ebULL);
}

TEST(ShipHashCase, test_24_one_mio_zero_bytes) {
  uint64_t digest64;
  uint8_t msg[1000000] = { };

  digest64 = SipHash::Hash24(SPEC_KEY, msg, 1000000);
  EXPECT_EQ(digest64, 0x28205108397aa742ULL);
}

TEST(ShipHashCase, test_24_reference_vectors) {
  uint64_t digest64;
  uint8_t msg[64];
  uint8_t digest[sizeof(uint64_t)];
  for(int i = 0; i < 64; ++i) {
    msg[i] = i;
    digest64 = SipHash::Hash24(SPEC_KEY, msg, i);
    U64TO8_LE(digest, digest64);
    EXPECT_TRUE(!memcmp(SPEC_VECTORS[i], digest, 8));
  }
}
