...
class Compiler {
 public:
  ...
  void EmitBINARY_SUBTRACT(const Instruction* instr) {
    ...
    type_record_.Put(dst, TypeEntry::Subtract(type_record_.Get(lhs), type_record_.Get(rhs)));
  }
  ...
  void EmitBINARY_LTE(const Instruction* instr, OP::Type fused = OP::NOP) {
    ...
    const int16_t rhs = Reg((fused == OP::NOP) ? instr[1].i16[2] : instr[1].jump.i16[1]);
    {
      const Assembler::LocalLabelScope scope(asm_);
      LoadVRs(asm_->rsi, lhs, asm_->rdx, rhs);
      Int32Guard(lhs, asm_->rsi, asm_->rax, ".BINARY_LTE_SLOW");
      Int32Guard(rhs, asm_->rdx, asm_->rax, ".BINARY_LTE_SLOW");
      asm_->cmp(asm_->esi, asm_->edx);
      ...
      asm_->mov(asm_->qword[asm_->r13 + dst * kJSValSize], asm_->rax);
      set_last_used_candidate(dst);
      type_record_.Put(dst, TypeEntry::LTE(type_record_.Get(lhs), type_record_.Get(rhs)));
    }
  }
  ...
  void EmitBINARY_GT(const Instruction* instr, OP::Type fused = OP::NOP) {
    ...
    const int16_t rhs = Reg((fused == OP::NOP) ? instr[1].i16[2] : instr[1].jump.i16[1]);
    {
      const Assembler::LocalLabelScope scope(asm_);
      LoadVRs(asm_->rsi, lhs, asm_->rdx, rhs);
      Int32Guard(lhs, asm_->rsi, asm_->rax, ".BINARY_GT_SLOW");
      Int32Guard(rhs, asm_->rdx, asm_->rax, ".BINARY_GT_SLOW");
      asm_->cmp(asm_->esi, asm_->edx);
      ...
      asm_->mov(asm_->qword[asm_->r13 + dst * kJSValSize], asm_->rax);
      set_last_used_candidate(dst);
      type_record_.Put(dst, TypeEntry::GT(type_record_.Get(lhs), type_record_.Get(rhs)));
    }
  }
  ...
  void EmitBINARY_GTE(const Instruction* instr, OP::Type fused = OP::NOP) {
    ...
    const int16_t rhs = Reg((fused == OP::NOP) ? instr[1].i16[2] : instr[1].jump.i16[1]);
    {
      const Assembler::LocalLabelScope scope(asm_);
      LoadVRs(asm_->rsi, lhs, asm_->rdx, rhs);
      Int32Guard(lhs, asm_->rsi, asm_->rax, ".BINARY_GTE_SLOW");
      Int32Guard(rhs, asm_->rdx, asm_->rax, ".BINARY_GTE_SLOW");
      asm_->cmp(asm_->esi, asm_->edx);
      ...
      asm_->mov(asm_->qword[asm_->r13 + dst * kJSValSize], asm_->rax);
      set_last_used_candidate(dst);
      type_record_.Put(dst, TypeEntry::GTE(type_record_.Get(lhs), type_record_.Get(rhs)));
    }
  }
  ...
  void EmitBINARY_EQ(const Instruction* instr, OP::Type fused = OP::NOP) {
    ...
    const int16_t rhs = Reg((fused == OP::NOP) ? instr[1].i16[2] : instr[1].jump.i16[1]);
    {
      const Assembler::LocalLabelScope scope(asm_);
      LoadVRs(asm_->rsi, lhs, asm_->rdx, rhs);
      Int32Guard(lhs, asm_->rsi, asm_->rax, ".BINARY_EQ_SLOW");
      Int32Guard(rhs, asm_->rdx, asm_->rax, ".BINARY_EQ_SLOW");
      asm_->cmp(asm_->esi, asm_->edx);
  ...
  void EmitBINARY_STRICT_EQ(const Instruction* instr, OP::Type fused = OP::NOP) {
    ...
    const int16_t rhs = Reg((fused == OP::NOP) ? instr[1].i16[2] : instr[1].jump.i16[1]);
    {
      ...
      Int32Guard(lhs, asm_->rdi, asm_->rax, ".BINARY_STRICT_EQ_SLOW");
      Int32Guard(rhs, asm_->rsi, asm_->rax, ".BINARY_STRICT_EQ_SLOW");
  ...
  void EmitBINARY_NE(const Instruction* instr, OP::Type fused = OP::NOP) {
    ...
    const int16_t rhs = Reg((fused == OP::NOP) ? instr[1].i16[2] : instr[1].jump.i16[1]);
    {
      const Assembler::LocalLabelScope scope(asm_);
      LoadVRs(asm_->rsi, lhs, asm_->rdx, rhs);
      Int32Guard(lhs, asm_->rsi, asm_->rax, ".BINARY_NE_SLOW");
      Int32Guard(rhs, asm_->rdx, asm_->rax, ".BINARY_NE_SLOW");
      asm_->cmp(asm_->esi, asm_->edx);
  ...
  void EmitBINARY_STRICT_NE(const Instruction* instr, OP::Type fused = OP::NOP) {
    ...
    const int16_t rhs = Reg((fused == OP::NOP) ? instr[1].i16[2] : instr[1].jump.i16[1]);
    {
      const Assembler::LocalLabelScope scope(asm_);
      LoadVRs(asm_->rdi, lhs, asm_->rsi, rhs);
      Int32Guard(lhs, asm_->rdi, asm_->rax, ".BINARY_STRICT_NE_SLOW");
      Int32Guard(rhs, asm_->rsi, asm_->rax, ".BINARY_STRICT_NE_SLOW");
      asm_->cmp(asm_->esi, asm_->edi);
  ...
  void EmitUNARY_BIT_NOT(const Instruction* instr) {
    ...
    const int16_t src = Reg(instr[1].i16[1]);
    {
      const Assembler::LocalLabelScope scope(asm_);
      LoadVR(asm_->rsi, src);
      Int32Guard(src, asm_->rsi, asm_->rax, ".UNARY_BIT_NOT_SLOW");
      asm_->not(asm_->esi);
      asm_->mov(asm_->eax, asm_->esi);
      asm_->or(asm_->rax, asm_->r15);
