...
// 
// added small modification for iv project
//  rm boost dependency
//  mv to namespace iv::core
//
#ifndef IV_DETAIL_UNIQUE_PTR_H_
...
public:
    ...
    unique_ptr()
        {
            IV_STATIC_ASSERT(!std::is_reference<deleter_type>::value);
            IV_STATIC_ASSERT(!std::is_pointer<deleter_type>::value);
        }
    ...
    unique_ptr(pointer p, typename iv::if_<std::is_reference<D>,
...
private:
    ...
    template <class U>
        unique_ptr(U, typename iv::if_<std::is_reference<D>,
